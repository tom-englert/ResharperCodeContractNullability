...
namespace CodeContractNullability.ExternalAnnotations
{
    ...
    /// </remarks>
    public sealed class FolderExternalAnnotationsLoader
    {
        [NotNull]
        ...
        private ExternalAnnotationsCache TryGetCacheFromDisk()
        {
            try
            {
                if (fileSystem.File.Exists(CachePath))
                {
                    ...
                    using (Stream stream = fileSystem.File.OpenRead(CachePath))
                    {
            ...
            foreach (string folder in scanner.GetFoldersToScan())
            {
                if (fileSystem.Directory.Exists(folder))
                {
                    foreach (string path in
                        fileSystem.Directory.EnumerateFiles(folder, "*.xml", SearchOption.AllDirectories))
                    {
                        fileSet.Add(path);
                    }
                }
        ...
        private sealed class HighestLastWriteTimeUtcRecorder
        {
            ...
            {
                IFileInfo fileInfo = fileSystem.CreateFileInfo(path);
